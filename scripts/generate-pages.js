...
function seoContent(placeName, isSubRegion) {
  const fullPlace = isSubRegion ? `${city} ${placeName}` : `${city} ${placeName}`;
  return `
    <h3>${fullPlace} 유품정리와 폐기물처리, 어떻게 진행되나요?</h3>
    <p>${fullPlace}에서는 유품정리, 고독사청소, 특수청소와 함께 쓰레기집청소, 빈집정리, 가정·이사·폐업폐기물처리 상담이 많이 접수됩니다. 아파트, 빌라, 단독주택, 상가, 농가 등 현장 형태에 따라 작업 방식과 비용이 달라지므로 사전 상담이 중요합니다.</p>
    <h3>${fullPlace} 유품정리 비용은 어떻게 결정될까요?</h3>
    <p>유품정리 비용은 평수만으로 정해지지 않습니다. 유품의 양, 폐기물 반출량, 가구·가전 크기, 엘리베이터·주차 환경, 특수청소 필요 여부가 함께 반영됩니다. ${isSubRegion ? `${placeName}은` : `${placeName} 일대는`} 신도시·아파트 단지와 주택·농가가 혼재해 반출 동선 확인이 특히 중요합니다.</p>
...
<main>
  ...
  <section id="seo">
    <div class="wrap">
      ...
      <div class="seo-article">${seoContent(placeName, isSubRegion)}</div>
    </div>
  </section>
